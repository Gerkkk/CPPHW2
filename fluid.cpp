...
int main(int argc, char* argv[]) {
    std::string p_type, v_type, v_flow_type, file_name, out_c;
    ...
    for (int i = 1; i < arguments.size(); ++i) {
        ...
        else if (std::string(arguments[i]) == "--out-count") {
            if (i + 1 < arguments.size()) {
                out_c = arguments[i + 1];
                ++i;
            } else {
                std::cerr << "Error: --out-c requires a value." << std::endl;
                return 1;
            }
        } else {
